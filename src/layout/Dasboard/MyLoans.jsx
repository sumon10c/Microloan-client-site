...
const MyLoans = () => {
    ...
    if (isLoading) {
        return <div className="flex justify-center items-center min-h-[400px]"><span className="loading loading-spinner loading-lg text-primary"></span></div>;
    }
        ...
        <div className="p-6 lg:p-10 bg-[#F8FAFC] min-h-screen">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
                    <div>
                        <h2 className="text-3xl font-black text-slate-900 tracking-tight">My Loan Applications</h2>
                        <p className="text-slate-500 font-medium">Track and manage your requested loans</p>
                    </div>
                    <div className="bg-white px-6 py-3 rounded-2xl shadow-sm border border-slate-100">
                        ...
                        <span className="text-2xl font-black text-blue-600">{loansApplication.length}</span>
                    </div>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="table w-full border-separate border-spacing-y-0">
                            ...
                            <thead className="bg-slate-50">
                                <tr>
                                    <th className="py-5 px-6 text-[11px] font-black text-slate-400 uppercase tracking-widest border-none">Loan Purpose</th>
                                    <th className="py-5 px-6 text-[11px] font-black text-slate-400 uppercase tracking-widest border-none">Amount</th>
                                    <th className="py-5 px-6 text-[11px] font-black text-slate-400 uppercase tracking-widest border-none">Duration</th>
                                    <th className="py-5 px-6 text-[11px] font-black text-slate-400 uppercase tracking-widest border-none">Applied Date</th>
                                    <th className="py-5 px-6 text-[11px] font-black text-slate-400 uppercase tracking-widest border-none">Status</th>
                                </tr>
                            </thead>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {loansApplication.length > 0 ? (
                                    loansApplication.map((loan) => (
                                        <tr key={loan._id} className="hover:bg-blue-50/30 transition-colors group">
                                            <td className="py-5 px-6">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[10px] text-slate-400 font-medium uppercase tracking-tighter">ID: {loan._id.slice(-8)}</p>
                                                    </div>
                                                </div>
                                            </td>
